...
This fork ships a single-container setup that serves the client and the game
websocket on one port, with Redis as a separate service. It works behind a
TLS-terminating reverse proxy (the client auto-detects `wss://` from the page
origin), so it deploys cleanly on Coolify.

Run it locally with Docker Compose (app + Redis):

    $ docker compose up --build
    # then open http://localhost:8000

Deploy on Coolify (two options):

1. **Docker Compose resource** — point Coolify at this repo and select
   `docker-compose.yml`. It provisions both the game and Redis. Set the public
   domain on the `browserquest` service; Coolify terminates TLS for you.

2. **Dockerfile (app) + managed Redis** — create a Redis resource in Coolify,
   then deploy this repo with the Dockerfile build pack and set these env vars
   on the app:

   * `PORT` — usually injected by Coolify (defaults to `8000`)
   * `REDIS_HOST` — hostname of the Redis service (**required** to activate the
...
* Runtime is Node 10 (newest the socket.io 1.x stack runs cleanly on). The only
  former native dependency, `bcrypt`, was swapped for the pure-JS `bcryptjs`, so
  the image needs no build toolchain.
* The server stays up even if Redis is unreachable (errors are logged); login
  and persistence simply won't work until Redis is connected.
* Health check hits the built-in `/status` endpoint.

How to get it going (local, without Docker)
-------------------------------------------
