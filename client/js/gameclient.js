...
define(['player', 'entityfactory', 'lib/bison'], function(Player, EntityFactory, BISON) {
    ...
    var GameClient = Class.extend({
        ...
        connect: function(dispatcherMode) {
            // Derive the websocket scheme/host from the page's own origin so the
            // game works behind a TLS-terminating reverse proxy (e.g. Coolify).
            // On an https page we must use wss:// or the browser blocks it as
            // mixed content; window.location.host already carries any non-default
            // port (e.g. "localhost:8000") for local single-port dev.
